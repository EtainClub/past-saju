...
| --- | :---: | ---: |
| **Opus 5** | **3/3** | 16.8s |
| Haiku 4.5 | 1~3 중 1~2 (두 번 측정) | 8.1s |

두 배 빠르지만 **전환점 개월 누락**으로 절반 가까이 떨어집니다. 재시도 1회를 감안해도 폴백률이 **25% 안팎** — 넷 중 하나가 템플릿을 받으면 A4를 도입한 이유가 없어집니다.

같은 입력(보통 서술)의 문장을 나란히 보면 차이가 더 분명합니다:

> **Opus** — "…두 달의 고민이 떠나는 쪽으로 기울었다면, **명식의 국면은 전환에 놓이고 동료·독립 영역이 먼저 움직였을 겁니다.** 조직이 정해 준 자리 대신 아직 형태가 없는 자리에 들어가는 출발이었을 가능성이 읽힙니다."
>
> **Haiku** — "…새로운 조직 속에서 자신의 표현 방식을 처음부터 만들어내는 과정이 시작되었을 것입니다. 익숙한 역할과 관계의 틀을 버리고 낯선 환경에 몸을 맡기는 선택이었을 **테입니다**."

Opus는 국면과 영역을 문장에 싣습니다. Haiku는 **사용자 서술을 되풀이할 뿐 명식이 들어 있지 않습니다** — "사주가 고른 세 개의 길"이라는 전제가 사라집니다. `테입니다`(→터입니다) 오탈자도 있습니다.

**재검토 조건:** 개월 언급을 프롬프트가 아니라 **코드가 보장**하도록 바꾸면(§5.4-a의 대가 문장처럼) Haiku의 주 실패 원인이 사라집니다. 그때 다시 잽니다.
...
첫 구현이 "**첫 번째 통과 샘플**"의 산문을 찍었습니다. Opus는 1번이 통과하고 Haiku는 1번이 떨어져, **서로 다른 입력의 문장을 나란히 놓고 품질을 비교**하고 있었습니다. 표본 인덱스를 고정하고 통과·탈락을 함께 표시하도록 고쳤습니다.

`--sweep`의 캐시 예열 누락(§5.C)과 같은 종류의 실수입니다 — **비교에서는 비교 대상 말고 전부 고정해야 합니다.**
...
3. **지연 17초** — `effort`로는 안 줄어듭니다(§5.C에서 반증). 산문 길이나 모델을 건드려야 하므로 **실사용 이탈률을 보고** 결정합니다. 카드 공개는 즉시이므로 체감이 다를 수 있습니다.
4. 나머지 §7-A(2·3·5)는 해당 층 작업 시점에 확정합니다.
