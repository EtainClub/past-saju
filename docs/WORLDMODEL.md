...
| **B2** ◐ | 골든 케이스 43건 — 골격·러너 완료, 1차 대조 완료 | **37/43 통과**(엔진 버그 0건). 남은 6건 대조 후 공개 — §5.B-c 참조 |
...
| L1 정확성 | 골든 43건 100% (현재 37/43) | `scripts/chart-golden.ts` |
...
A0~A4와 B1이 끝났고, B2는 37/43입니다. 남은 것은 배포 검증뿐입니다.

1. **골든 나머지 6건 대조** — 음력 5건(각 케이스 `note`의 **양력 환산일**을 쓸 것) + `regular-6`. 43/43이 되면 B2가 닫히고 A4를 100%로 올릴 수 있습니다. **공개 전 유일한 필수 항목입니다.**
2. **LLM 단가 실측** — `count_tokens`로 세션당 입력·출력 토큰을 재고 §7-8(예산 상한)을 확정합니다. 베타 규모를 정하려면 이 값이 먼저 필요합니다.
3. **지연 단축** — 현재 16~22초. `effort: low` 대비 품질을 재고 결정합니다(§5.4-a).
