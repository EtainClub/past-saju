...
### 5.E. 단가 확정 — §7-8 종료 (2026-08-17)

**앞선 모든 금액 추정이 6배 과대평가였습니다.** 두 오류가 겹쳤습니다.

| 오류 | 배수 |
| --- | :---: |
| 단가표를 **Opus 4.1(퇴역)의 $15/$75**로 가정. Opus 5는 **$5/$25** | 3× |
| 모델 교체 전(Opus+Opus) 기준으로 계산 | 2× |

토큰 수는 처음부터 실측이었고 틀리지 않았습니다. **금액만 틀렸고, 그건 제가 확인하지 않은 외부 사실을 가정으로 채운 결과입니다.** 그래서 출력에 "단가 미확인" 경고를 달아 두었고, 실제로 그 경고가 가리키던 것이 틀렸습니다.

**확정 단가** (USD / 1M tok, 요금 페이지 2026-08-17):

| 모델 | 입력 | 출력 | 캐시쓰기(5m) | 캐시읽기 |
| --- | ---: | ---: | ---: | ---: |
| Opus 5 | $5 | $25 | $6.25 | $0.50 |
| **Sonnet 5** | **$2** | **$10** | $2.50 | $0.20 |
| **Haiku 4.5** | **$1** | **$5** | $1.25 | $0.10 |

`src/lib/llm/pricing.ts`가 단일 출처입니다. 모델 id의 날짜 접미사를 접두사 매칭으로 흡수하므로 스냅샷이 바뀌어도 표를 안 고쳐도 됩니다. **단가를 모르는 모델은 `null`을 돌려줍니다 — `0`을 돌려주면 "공짜"로 읽혀 더 나쁩니다.**

**세션당 실측 비용** (L2 Haiku + L5 Sonnet):

| 케이스 | L2 | L5 | 세션 |
| --- | ---: | ---: | ---: |
| 짧은 서술 | $0.0018 | $0.0167¹ | $0.0185 |
| 보통 서술 | $0.0024 | $0.0112 | **$0.0136** |
| 긴 서술(폼 상한) | $0.0031 | $0.0128 | $0.0159 |

¹ 첫 호출이라 캐시 **쓰기**를 물었습니다. 정상 운영은 읽기라 더 쌉니다.

**모델 교체 효과** — 보통 서술 기준 Opus+Opus였다면 약 $0.036, 지금은 **$0.0136으로 62% 절감**입니다.

**§7-8 답 — 하루 500 호출 상한**

| 시나리오 | 세션/일 | 일 지출 |
| --- | ---: | ---: |
| 패턴 전부 미스 (세션당 2호출) | 250 | $3.91 |
| 패턴 전부 히트 (세션당 1호출) | 500 | $7.81 |
| 최악 표본 기준 | 500 | $10.43 |

**월 $117 ~ $313** (약 16만~43만원). 베타 규모라면 오히려 **상한이 낮은 편**이므로, 실트래픽을 보고 올릴 여지가 있습니다.

**L2 캐시 무용화의 손익** — Haiku 입력이 $1/Mtok이라 캐시 미적용 손실은 세션당 **약 $0.0014**(전체의 10%)입니다. 프롬프트를 억지로 늘려 최소 길이를 맞추는 것은 **하루 500호출에서 $0.70 차이**라 할 이유가 없습니다. 트래픽이 자릿수로 커지면 재검토합니다.
...
8. ~~**LLM 예산**~~ — **실측 완료(2026-08-17): 세션당 $0.014, 하루 500호출 상한 = 월 $117~$313.** 근거는 §5.E. 상한 자체를 올릴지는 실트래픽을 보고 정합니다.
...
1. ~~**단가표 확정**~~ — **완료(§5.E).** 세션당 $0.014, 하루 500호출 상한은 월 $117~$313입니다.
...
4. ~~**L2 캐시 무용화**~~ — **손익 계산 완료(§5.E): 세션당 $0.0014, 하루 500호출에서 $0.70.** 손댈 이유가 없습니다. 트래픽이 자릿수로 커지면 재검토합니다.
