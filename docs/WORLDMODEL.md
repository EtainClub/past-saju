...
| 자시 처리 | `dayBoundary: 'midnight'\|'jasi'\|'splitJasi'`. 현재 `'splitJasi'` (야자시설, `chart/profile.ts`) |
...
| **B2** ◐ | 골든 케이스 43건 — 골격·러너 완료, **정답값은 사람이 만세력 대조** | 현재 0/43 검증. 공개 전 필수 — §5.B-a 참조 |
| **B3** | `chart/adapter.ts` 분리 + `meta.solarTermBoundary` + §6-3·§6-6 수정 | — |

#### 5.B-a. B1·B2 구현 기록 (2026-08-16)

**§7-1 자시 처리 — `splitJasi`(야자시설)로 결정**
...
**만세력 설정을 맞춰야 합니다** (안 맞추면 전부 틀린 것처럼 나옵니다):

- **진태양시 적용**, 출생지 서울(126.978°) — 끄면 시주가 약 30분 어긋나 경계 케이스가 전부 불일치합니다
- **야자시/조자시 구분함** (`splitJasi`)
- **서머타임·과거 표준시 자동 보정**

**현재 0/43.** `pnpm test`에 편입되어 있으며, 미검증 케이스는 실패가 아니라 "미검증"으로 집계됩니다. 불일치가 나오면 라이브러리 버그일 수 있으므로, 우리 코드를 고치기 전에 어느 쪽 문제인지 먼저 분리 리포트합니다.
...
1. ~~**자시 처리**~~ — **결정 완료(2026-08-16): `splitJasi`(야자시설).** 23시대 출생은 일주 당일 유지, 시주 천간만 다음날 일간 기준. 근거와 되돌리는 법은 §5.B-a.
