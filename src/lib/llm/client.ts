 ...
 *      Opus를 쓸 이유가 없다.
 *
 * L5 = Sonnet 5 — Opus와 충실성이 같고(각 6/6) 33% 빠르다(17.5s → 12.0s).
 *      Haiku는 여기서 탈락했다: 충실성 1~2/3(전환점 개월 누락)이라 재시도를
 *      감안해도 넷 중 하나가 템플릿을 받는다. A4를 도입한 이유가 사라진다.
 *
 *      Sonnet은 충·합 같은 명리 용어를 자연어로 풀지 않고 그대로 노출하는
 *      편이다(Opus는 "부딪히며 열리는"으로 푼다). 거슬리면 프롬프트에
 *      한 줄 더하면 된다 — 모델을 되돌릴 일은 아니다.
 */
const DEFAULT_MODEL = {
